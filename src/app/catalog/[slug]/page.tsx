...
export async function generateMetadata({ params }: { params: { slug: string } }) {
    ...
    const product = await fetch(`https://wdxu2yyw.api.sanity.io/v2023-08-10/data/query/production?query=*[_type == 'products']`).then((res: any) => res.json());
...
async function fetchPreviewData(slug: string) {
    let res = await fetch(`https://wdxu2yyw.api.sanity.io/v2023-08-10/data/query/production?query=*%5B_type+%3D%3D+%27products%27+%26%26+slug.current+%3D%3D+%22${slug}%22%5D`)
    return res.json();
...
export async function generateStaticParams() {
    let res = await fetch(`https://wdxu2yyw.api.sanity.io/v2023-08-10/data/query/production?query=*[_type == 'products']`, {
        next: {
            revalidate: 60
        }
